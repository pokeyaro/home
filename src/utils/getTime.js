...
export const siteDateStatistics = (startDate) => {
  ...
  if (differenceInYears >= 1) {
    return `小 Po 站已经苟活了 ${Math.floor(differenceInYears)} 年 ${Math.floor(
      differenceInMonths % 12
    )} 月 ${Math.round(differenceInDays % 30)} 天`;
  } else if (differenceInMonths >= 1) {
    return `小 Po 站已经苟活了 ${Math.floor(differenceInMonths)} 月 ${Math.round(
      differenceInDays % 30
    )} 天`;
  } else {
    return `小 Po 站已经苟活了 ${Math.round(differenceInDays)} 天`;
  }
};
